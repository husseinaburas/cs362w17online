...
int mineCard(int handPos, int choice1, int choice2, int currentPlayer, struct gameState *state){
    ...
    for (i = 0; i < state->handCount[currentPlayer]; i++)
	{
	    ...
	    {
            discardCard(i, currentPlayer, state, 0); //should this be 1??
            break;
	    }
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      return (adventurerCard(temphand, cardDrawn, currentPlayer, state));
    ...
    case mine:
      return (mineCard(handPos, choice1, choice2, currentPlayer, state));
    ...
    case baron:
      return (baronCard(currentPlayer, choice1, state));
    ...
    case minion:
      //+1 action
      return (minionCard(choice1, choice2, currentPlayer, handPos, state));
