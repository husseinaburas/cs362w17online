...
int main( int argc, char** argv )
{
	...
	printf ("\n----------Starting Random Card Test: Smithy----------\n\n");
	...
	for(i = 0; i < MAX_TESTS; i++)
	{
		...
		//Guarantee that there is at least one smithy card
		int smithyPos = rand() % G->handCount[0];
		...
		returnValue = playSmithy(G, 0, smithyPos);
		numBugs += customAssert( (returnValue == 0), "Call to playSmithy() failed." );
		...
		//Since smithy card was played, need to increment this counter by 1
		exp_playedCardCount++;
		...
		//Intended behavior: draw 3 cards, discard 1 (smithy), so net change is +2 cards
		int numCardsDrawn = G->handCount[0] - exp_handCount[0];

		numBugs += customAssert( (numCardsDrawn == 2), "Incorrect number of cards drawn." );


/* Test abandoned because in the case where the last card drawn is also a smithy, it will be 
 * replaced with the original card
 * Without a way to modify the playSmithy() function directly, there is no way to distinguish
 * between improper discarding and the case where the third card drawn (i.e. end of hand) is simply
 * swapped with the played smithy card, as intended 
		//Smithy card should be discarded
		//adjacentSmithy tracks if the next card (prior to calling playSmithy()) is also a smithy
		//card. If so, it helps avoid incorrectly throwing an error
		if (!adjacentSmithy)
		{
			numBugs += customAssert( (G->hand[0][smithyPos] != smithy), 
						"Smithy card was not discarded correctly");
		}
*/
	...
	if(numBugs)
	{
		printf("Random Card Test: Smithy - Summary\n");
	...
	{
		printf("\nRandom Card Test: Smithy - All tests for playSmithy() successful\n");
	}

	printf("\n----------End of Random Card Test: Smithy----------\n");
