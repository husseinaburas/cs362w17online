...
// Description: This program will run random tests for the great_hall card
...
int main(int argc, char** argv){
    ...
    // find the location of an great_hall card
    int positionGreatHall;
	...
	while(testCount < NUM_TESTS){
		// set great_hall position to -1 (position not found)
		positionGreatHall = -1;
		...
		// find the great_hall card in the player's hand
		for(j = 0; j < rHandCount; j++){
			if(game->hand[currentPlayer][j] == great_hall && positionGreatHall == -1){
				positionGreatHall = j;				
			}
		}
							
		// make a copy of game before playing great_hall
		memcpy(&pre, game, sizeof(struct gameState));
				
		// if the player has an great_hall card in their hand, play the card 
		if(positionGreatHall != -1){
			// set addedCoinSum to 0
			addedCoinSum = 0;
					
			// play the great_hall card
			cardEffect(great_hall, 0, 0, 0, game, positionGreatHall, 0);
			...
			// check if the playedCard was incremented after playing great_hall
			...
			// check if the correct number of cards was removed from the players hand after playing great_hall	
			...
			if(assertRet){
				printf("Test %i Failed (Removed great_hall Card)\n", checkNumber + 1 + (testCount*numChecks));
				checkFailureCount = checkFailureCount + 1;
				checkArr[checkNumber] = 1;
			}
			...
			assertRet = assertTrue(game->numActions, pre.numActions + 1,checkNumber + 1 + (testCount*numChecks), NUM_TESTS * numChecks, &numPass, &numFail);
			...
			assertRet = assertTrue(game->hand[currentPlayer][positionGreatHall], pre.deck[currentPlayer][pre.deckCount[currentPlayer]-1],checkNumber + 1 + (testCount*numChecks), NUM_TESTS * numChecks, &numPass, &numFail);
			...
			if(checkFailureCount >= 1){
				...
				printf("\nGAME STATE PRIOR TO PLAYING GREAT HALL\n"); 
				printGameState(&pre, currentPlayer);
				printf("\nGAME STATE AFTER TO PLAYING GREAT HALL\n"); 
