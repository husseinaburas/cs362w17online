...
int adventure(int dt,struct gameState *st,int cp, int cd,int th[],int zv)
{
	while(dt<2)
	{
        	if (st->deckCount[cp] <1)
		{//if the deck is empty we need to shuffle discard and add to deck
        		shuffle(cp, st);
        	}
        	drawCard(cp, st);
        	cd = st->hand[cp][st->handCount[cp]-1];//top card of hand is most recently drawn card.
        	if (cd == copper || cd == silver || cd == gold)
        		dt++;
        	else
		{
			zv++; 
...
int smithi(int drawntreasure,struct gameState *state,int currentPlayer,int cardDrawn,int temphand,int z,int handPos, int choice1,int choice2)
{
    ...
    //discard card from hand
    printf("perhaps discarding cards?\n");
    discardCard(handPos, currentPlayer, state, 0);
    return 1;
}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      	return adventure(drawntreasure,state,currentPlayer,cardDrawn,temphand,z);
			
    case council_room:
      	return counsilRoom(drawntreasure,state,currentPlayer,cardDrawn,temphand,z,handPos);
    case feast:
    ...
    case mine:
      return myne(drawntreasure,state,currentPlayer,cardDrawn,temphand,z,handPos,choice1,choice2);
    ...
    case smithy:
		return smithi(drawntreasure,state,currentPlayer,cardDrawn,temphand,z,handPos,choice1,choice2);
    case village:
        return villag(drawntreasure,state,currentPlayer,cardDrawn,temphand,z,handPos,choice1,choice2);
