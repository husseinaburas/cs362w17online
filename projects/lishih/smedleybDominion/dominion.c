...
int playVillage(struct gameState *state, int handPos, int currentPlayer){
	//+1 Card
      	...
      	//+2 Actions
      	state->numActions = state->numActions * 2; // fix bug change numAction*2 to numAction+2
...
//gain card with cost up to 5
    ...
    /*  int x = 1 ; //fix bug, int x=1;*/
      while( i == 1) {//Buy one card , fix bug i ==1 to x==1
	if (supplyCount(choice1, state) <= 0){
	  ...
	  i = 0;//No more buying cards, fix bug change i to x
