...
int main (){
    ...
    // pass the copy game to the smithy card effect, play the smithy
    cardEffect(smithy, choice1, choice2, choice3, &testCardEffect, handpos, &bonus);

    /*--------------------Output Result: check player's hand count and deck count---------------*/
    printf("Test result: current player status:\n");
    printf("Hand Count = %d, Expected = %d\n", testCardEffect.handCount[currentPlayer], C.handCount[currentPlayer] + drawNewCards);
