...
int main() {
	...
	printf("****** Testing Method <playFeast  ;>******\n");
	...
	playFeast(state1,playerIndex);
	int isCurseInOpponentDeck = 0;

	int playerIndex2 = 1;

	for (j = 0; j < state1->deckCount[playerIndex2]; j++) {
		if (state1->deck[playerIndex2][j] == curse) {
			isCurseInOpponentDeck = 1;
		}
	}

	assertMsg("Testing adding curse to opponent Deck", isCurseInOpponentDeck,
			1);

	int isCurseInCurrentPlayerDeck = 1;
	for (j = 0; j < state1->deckCount[playerIndex]; j++) {
		if (state1->deck[playerIndex][j] == curse) {
