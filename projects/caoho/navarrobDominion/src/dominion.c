	...
	for (i = adventurer; i <= treasure_map; i++)  //loop all cards
		...
		for (j = 0; j < 10; j++)    //loop chosen cards
			...
			if (kingdomCards[j] == i) {
				...
				if (kingdomCards[j] == great_hall
						|| kingdomCards[j] == gardens) {
					if (numPlayers == 2) {
	...
	if (state->numBuys < 1) {
		...
		if (DEBUG)
			printf(
					...
					supplyPos, getCost(supplyPos), state->numBuys,
					state->coins);
	}
...
//		if (state->deckCount[currentPlayer] <= 1) { //navarrob's bug
		if (state->deckCount[currentPlayer] < 1) { //my fix
...
int playCouncil_Room(struct gameState *state, int handPos) {
	...
	// Draw 4 cards
//	for (i = 0; i <= 4; i++) {  // navarrob' bug
	for (i = 0; i < 4; i++){ // my fix
		drawCard(currentPlayer, state);
	}

	// navarrob' bug failed to add the number of buy;
	state->numBuys++;
