...
void testAdventurerCard(int currentPlayer, struct gameState *state)
{
	...
	printf("Current player: %d, hand count after: %d (expected %d), deck count after: %d (expected %d)\n", currentPlayer, (state->handCount[currentPlayer]), (handCtpre + 2), (state->deckCount[currentPlayer]), (deckCtpre - disCardTot - 2)); 
    }
}
