...
int main() {
	...
	printf("Actions before using smithy: %i\n", action_before);
	card_before = numHandCards(&G);
	printf("Cards in hand before using smithy: %i\n", card_before);
	
	//use card
	cardEffect(smithy,0,0,0,&G,4,0);
	
	action_after=G.numActions;
	printf("Actions after using smithy: %i\n", action_after);
	card_after = numHandCards(&G);
	printf("Cards in hand after using smithy: %i\n", card_after);
