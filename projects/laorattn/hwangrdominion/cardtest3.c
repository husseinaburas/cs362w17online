...
int main() {
	...
	printf("Actions before using Village: %i\n", action_before);
	card_before = numHandCards(&G);
	printf("Cards in hand before using Village: %i\n", card_before);
	
	//use card
	cardEffect(village,0,0,0,&G,4,0);
	
	action_after=G.numActions;
	printf("Actions after using Village: %i\n", action_after);
	card_after = numHandCards(&G);
	printf("Cards in hand after using Village: %i\n", card_after);
	
	//compare results
	assert_check=myassert(action_before == (action_after-2) && ": failed\n");
