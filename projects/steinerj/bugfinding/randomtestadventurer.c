...
{ 
  ...
  int randomPlayer, randomDrawn, randomInitSeed, randomHandPos, preTreasure, postTreasure, preHandCount, postHandCount;
  ...
  for (n = 0; n < 2000; n++) 
  {
	...
	randomDrawn = floor(Random() * 26);   //numbers for all cards????
	int tempHand[MAX_HAND];
	int t;
	for(t = 0; t < game.handCount[randomPlayer]; t++)
	{
		if (game.hand[randomPlayer][t] == copper || game.hand[randomPlayer][t] == silver

			 || game.hand[randomPlayer][t] == gold) 
		{
			preTreasure++;
		}
	}
	preHandCount = game.handCount[randomPlayer];
	adventurerFunc(randomHandPos, &game, randomPlayer, randomDrawn, tempHand);
	int u;
	for(u = 0; u < game.handCount[randomPlayer]; u++)
	{
		if (game.hand[randomPlayer][u] == copper || game.hand[randomPlayer][u] == silver

			 || game.hand[randomPlayer][u] == gold) 
		{
			postTreasure++;
		}
	}
	...
	{
		printf("postTreasure is %i preTreasure is %i\n", postTreasure, preTreasure);
		printf("test %i failed. Player didn't receive 2 additional treasure\n", n);
		return 0;
	}
	...
	if(postHandCount - preHandCount != 2)
	{
		...
		printf("test %i failed. Player's hand wasn't increased by 1\n", n);
		return 0;
	}
	if(compareStates(&game, &game2, randomPlayer) != 0)
	{
		printf("state was changed outside of card's changes\n");
	}
  }
...
int compareStates(struct gameState *originGame, struct gameState *modGame, int currentPlayer)
{
	...
	// these should be changed by adventurer
