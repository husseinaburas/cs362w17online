...
int allTests(int numP, int c1, int c2, int c3, int handP, int bonus, int seed)
{
	//printf("testing with %i players, choices %i, %i, %i, hand position %i, bonus %i and seed %i\n", numP, c1, c2, c3, handP, bonus, seed);
	struct gameState G;
	struct gameState G2;
	...
	cardEffect(smithy, c1, c2, c3, &G2, handP, &bonus);

	//printf("----------Testing if player drew 3 cards from deck ----------\n");
	int pDeckChange;
	int pHandChange;
	pDeckChange = deckCountChange(0, &G, &G2);
	pHandChange = handCountChange(0, &G, &G2);
	if (pDeckChange == -3 && pHandChange == 2)
	{
	//	printf("----------Passed----------\n");
	}
	else
	{
		printf("----------Failed: didn't draw 3 cards----------\n");
	}
	...
	{
		printf("----------Failed: inactive players's states changed----------\n");
	}
