...
int allTests(int numP, int c1, int c2, int c3, int handP, int bonus, int seed)
{
	...
	cardEffect(village, c1, c2, c3, &G2, handP, &bonus);

	//printf("----------Testing if player has 2 additional actions----------\n");
	int pNumActions;
	pNumActions = compareNumActions(&G, &G2);
	if (pNumActions == 2)
	{
		//printf("----------Passed----------\n");
	}
	...
	{
		printf("----------Failed: Player's actions were not increased by 2----------\n");
	}

	//printf("----------Testing if player drew a card----------\n");
	int pDeckChange;
	int pHandChange;
	pDeckChange = deckCountChange(0, &G, &G2);
	pHandChange = handCountChange(0, &G, &G2);

	if (pDeckChange == -1 && pHandChange == 0) //right? add 1 and discard village?
	{
		//printf("----------Passed----------\n");
	}
	else
	{
		printf("----------Failed: player didn't draw 1 card----------\n");
	}
	...
	{
		printf("----------Failed: Victory or Kindom piles changed----------\n");
	}
