...
int allTests(int numP, int c1, int c2, int c3, int handP, int bonus, int seed)
{
	...
	cardEffect(council_room, c1, c2, c3, &G2, handP, &bonus);

	//printf("----------Testing if other players drew a card----------\n");
	int i; 
	for(i = 0; i < G.numPlayers; i++) //for each player
	{
		if (i != 0) //skip active player
		{ 
			int handChange;
			int deckChange;
			handChange = handCountChange(i, &G, &G2);
			deckChange = deckCountChange(i, &G, &G2);
			if(handChange == 1 && deckChange == -1)
			{
				//printf("----------Passed----------\n");
			}
			else
			{
				printf("----------Failed: other player's didn't draw a card----------\n");
			}

		}
	}

	//printf("----------Testing if number of buys was increased by 1----------\n");
	int buysChange;
	buysChange = compareNumBuys(&G, &G2);
	if (buysChange == 1)
	{
	//	printf("----------Passed----------\n");
	}
	...
	{
		printf("----------Failed: number of buys wasn't increased by 1----------\n");
	}

	//printf("----------Testing if player drew 4 cards from deck----------\n");
	int pDeckChange;
	int pHandChange;
	pDeckChange = deckCountChange(0, &G, &G2);
	pHandChange = handCountChange(0, &G, &G2);
	if (pDeckChange == -4 && pHandChange == 3)
	{
	//	printf("----------Passed----------\n");
	}
	else
	{
		printf("----------Failed: player didn't draw 4 cards----------\n");
	}
...
int inactiveChange(int currentPlayer, struct gameState *originGame, struct gameState * modGame) //1 if other players change
{
	...
	for(i = 0; i < originGame->numPlayers; i++) //for each player
	{
		if (i != currentPlayer) //skip active player
		{
			...
			//if (handCountChange(i, originGame, modGame) == 1)
		//	{
		//		printf("an inactive player's deck count was changed \n");
		//	}
