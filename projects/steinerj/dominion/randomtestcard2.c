...
{ 
  ...
  int randomPlayer, randomHandPos, randomInitSeed, preHandCount, postHandCount, preActions, postActions;
  ...
  for (n = 0; n < 2000; n++) 
  {
	...
	villageFunc(randomPlayer, &game, randomHandPos);	
	postHandCount = game.handCount[randomPlayer];
	postActions = game.numActions;

	if(postHandCount - preHandCount != 0)
	{
		printf("postHandCount is %i preHandCount is %i\n", postHandCount, preHandCount);
		printf("test %i failed. Player's didn't draw a card or didn't discard Village\n", n);
		return 0;
	}
		if(postActions - preActions != 2)
	{
		printf("postActions is %i preActions is %i\n", postActions, preActions);
		printf("test %i failed. Player's wasn't given 2 additional actions\n", n);
		return 0;
	}
