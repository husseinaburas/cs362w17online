...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch(card) {
    ...
    case council_room:
     	return councilroomCard(state, currentPlayer, handPos);
      break;
			
	  case mine:
		  return mineCard(state, currentPlayer,handPos, choice1, choice2);
      break;
...
int discardCard(int handPos, int currentPlayer, struct gameState *state, int trashFlag)
{
  ...
  state->discardCount[currentPlayer]++; //BUG FIX
	
  //remove card from player's hand
  if ( handPos == (state->handCount[currentPlayer] - 1) && handPos > 0 ) 	//last card in hand array is played
    {
...
int mineCard(struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2)
{
	...
	j = state->hand[currentPlayer][choice1];  //store card(treasure) we will trash
    ...
    // flag originally supposed to be 2, 0 will send to discard pile
    gainCard(choice2, state, 0, currentPlayer);
