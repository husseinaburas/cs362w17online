...
int main (int argc, char** argv){
	if (argc != 2){
		printf("Usage: randomtestcard1 [RandomSeed]\n");
		return 1;
	}
	...
	for (j = 1; j <= test_length; j++){
		...
		result = cardEffect(smithy, 0, 0, 0, &testG, testLocation, NULL);
		if (result == 0){
			validCount++;
			if (PRINT_ALL)
	...
	for (j = 1; j <= test_length; j++){
		...
		currentPlayer = whoseTurn(&G);
		cardPlacement = rand()%G.handCount[currentPlayer]; //setting smithy in random place
