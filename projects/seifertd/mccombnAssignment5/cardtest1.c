...
int main () {
    ...
    for(p = 0; p < numPlayer; p++)
    {
        ...
        checkPass = PASSED;
        printf("\tPlayer receives exactly 2 cards into their hand: ");
        ...
        checkPass = PASSED;
        printf("\tPlayer discard count +1: ");
        if(G.discardCount[p] != testG.discardCount[p]+1)
            checkPass = FAILED;
        result(checkPass);
        if(checkPass == FAILED)
        {
            printf("\t\tInitial discardCount == %d\n", testG.discardCount[p]);
            printf("\t\tdiscardCount after Smithy == %d\n", G.discardCount[p]);
        }
