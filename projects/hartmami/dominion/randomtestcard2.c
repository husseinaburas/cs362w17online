...
**
**  Description:    randomtester for the card baron in dominion tests:
**
...
**  verify curPlayer has 1 actions
**  error verify curPlayer has 2 actions (bug actions not decremented)
**  verify baron is at the top of curPlayer's discard pile
...
**
**  finally a total count of all failed tests is output to randomtestcard2.out
**
***********************************************************************************************/
...
int main()
{
	...
	for(j=0; j < 1000; j++)
	{
	    ...
	    G.deck[curPlayer][7] = deck[curPlayer][7] = deckCard8;
			...
			G.numActions = 2;  // player 0 has 2 actions
			G.playedCardCount = 0;  // curPlayer has not played any cards
			G.coins = 0;  // player 0 has 0 coins
			G.numBuys = 0;  // player 0 has no buys
			...
			if( j == 0 && i == 0 )
			{
				printf("Player 0 has 2 actions:  ");
				testAssert(G.numActions == 2);  // run testAssert
				printf("\n");

				printf("Player 0 has not yet played a card:  ");
				testAssert(G.playedCardCount == 0);  // run testAssert
				printf("\n");

				printf("Player 0 has 0 coins:  ");
				testAssert(G.coins == 0);  // run testAssert
				printf("\n");

				printf("Player 0 has 0 buys:  ");
		...
		if( j == 0 )
		{
			printf("Verify Player 0 has 1 actions:  ");
			testAssert(G.numActions == 1);  // run testAssert
			printf("\n");

			printf("Verify error Player 0 has 2 actions:  ");
			testAssert(G.numActions == 2);  // run testAssert
			printf("\n");

			printf("Player 0 has 1 buys:  ");
			testAssert(G.numBuys == 1);  // run testAssert
			printf("\n");
			printf("numBuys is: %d \n\n", G.numBuys);

			printf("     Verify Player 0 has 2 buys (due to assignment2 bug):  ");
		...
		if( j == 0 )
		{
			printf("Player %d has %d coins:  ", curPlayer, G.coins);
			printf("\n");
		}
