...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  /**************************************************
  Refactored function calls by Gary Smith
  **************************************************/
	if (card == smithy)
  {
      handleSmithy(currentPlayer, state, handPos);
      return 0;
  }
  if (card == adventurer)
  {
      handleAdventurer(currentPlayer, state, handPos, drawntreasure, temphand);
      return 0;
  }
  if(card == village)
  {
    handleVillage(currentPlayer, state, handPos);
    return 0;
  }
  if(card == steward)
  {
    handleSteward(currentPlayer, state, handPos, choice1, choice2, choice3);
    return 0;
  }
...
/****************************************************************
 REFACTORED CODE BY GARY SMITH - CS362  Winter 2017  
****************************************************************/
int handleSmithy(int currentPlayer, struct gameState *state, int handPos)
{
  int i;
  //+3 Cards
  for (i = 0; i <= 3; i++)
