...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  if (state->numBuys < 1) {
    ...
    gainCard(supplyPos, state, 0, who);  // card goes in discard, this might be
                                         // wrong.. (2 means goes into hand, 0
                                         // goes into discard)
    ...
    if (DEBUG)
      printf(
          "You bought card number %d for %d coins. You now have %d buys and "
          "%d coins.\n",
...
int drawCard(int player, struct gameState *state) {
  ...
  if (state->deckCount[player] <= 0) {  // Deck is empty
    ...
    deckCounter = state->deckCount[player];  // Create a holder for the deck
                                             // count
...
int cardEffect(int card, int choice1, int choice2, int choice3,
  ...
  switch (card) {
    ...
    case feast:
      return playFeast(state, handPos, choice1);
...
int playAdventurer(struct gameState *state, int handPos) {
  ...
  while (drawntreasure < 2) {
    ...
    if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
      ...
      state
          ->handCount[currentPlayer]--;  // this should just remove the top card
  ...
  discardCard(handPos, currentPlayer, state, 0); // discard played card
  return 0;
}
