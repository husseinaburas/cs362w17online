...
int main(int argc, char** argv) {
  ...
  printf("\t=======\tBegin Random Testing of playAdventurer\t=======\n");
  ...
  for (i = 0; i < numOfTestCycles; i++) {
    ...
    // Setting random hand position for player to adventurer card
    randHandPos = rand() % G.handCount[currentPlayer];
    G.hand[currentPlayer][randHandPos] = adventurer;

    // Copying state to compare to for testing
    memcpy(&copyG, &G, sizeof(struct gameState));

    // Playing adventurer card and manually triggering coin set
    playAdventurer(&G, randHandPos);
    updateCoins(currentPlayer, &G, 0);

    // Testing proper behavior of adventurer
    // Testing deck count has changed
    if (G.deckCount[currentPlayer] != copyG.deckCount[currentPlayer]) {
      iterationFailed = 1;
      failedTests += 1;
    }
    // Testing player's hand count increased by 2
    ...
    // Deck has decreased by at least 2 cards
    if (G.deckCount[currentPlayer] < copyG.deckCount[currentPlayer] + 1) {
      iterationFailed = 1;
      failedTests += 1;
    }
  ...
  printf("\t=======\tEnd Random Testing of playAdventurer\t=======\n");
