 ...
 * 	several scenarios to test whether isGameOver can properly detect that the
 * 	Dominion game should end. A game ends if isGameOver return 1, otherwise, it
 * 	should return 0.
 */
#include "dominion.c"
#include "dominion.h"
...
int main(int argv, char** argc){
	...
	UNIT_ASSERT((isGameOver(testState) == 0), "TEST CASE:\tGame initialized, test that game isn't over\t\tRESULT:");
	...
	UNIT_ASSERT((isGameOver(testState) == 1), "TEST CASE:\tSet province cards to zero, game should end\t\tRESULT:");
	...
	UNIT_ASSERT((isGameOver(testState) == 1), "TEST CASE:\t3 Supply piles empty, game should be over\t\tRESULT:");
	...
	testState->supplyCount[province] = 0;
	UNIT_ASSERT((isGameOver(testState) == 1), "TEST CASE:\tBoth province empty & 3 supply piles empty\t\tRESULT:");
	...
	UNIT_ASSERT((isGameOver(testState) == 0), "TEST CASE:\tOnly 2 supply piles empty, game shouldn't end\t\tRESULT:");
