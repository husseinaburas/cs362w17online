...
int checkGainCard (struct gameState * G) {
	...
	for (p=0; p<G->numPlayers; p++) {
		...
		toFlag[p] = p; // player0 to discard, player 1 to deck, player 2 to hand, player3 to discard
		if (toFlag[p] >  2) { toFlag[p] = 0; } // can't be bigger than 2
	}
	...
	for (p=0; p< G->numPlayers; p++) {
		...
		int supplyFlag = 1; //default that there is enough supply to draw the card
		...
		// CHECK IF THE SUPPLY WAS CHANGED CORRECTLY
		if (supplyFlag) {
			...
			// if toFlag == 2 (hand), check if handCount increased 0
			if (toFlag[p] == 2) {
			...
			// if toFlag == 0 (discard), check if discard count increased by 0
			if (toFlag[p] == 0) {
