...
int checkUpdateCoins (struct gameState * G) {
	...
	// TODO: check if bonus is very large or very small!
	int bonus = -2147483648; // max INT is 2147483647, min -2147483648
	...
	for (p=0; p< G->numPlayers; p++) {
		...
		if (Gcontrol.coins == G->coins) {
			...
			printf ("player %d, FAILED coin calculation: testresponse: %d, controlresponse %d\n",
...
int myOwnUpdateCoins (int p, struct gameState * G, int bonus) {
	...
	*/
	...
	G->coins = coinTot;
	printf ("Control algorithm LOG Player %d KEY COIN STATS -> copper cards: %d, silver cards: %d, gold cards: %d, bonus added: %d\n",
...
int main () {
	...
	for (player = 2; player <= 3; player++) {
		...
		if (response < 0) { // then an error in initialize game
			printf ("Unittest2 SETUP ERROR: Failed to initialize game. num players:%d.\n", player);
			printGameState (&G);
		...
		for (deckCt = 9; deckCt < 10; deckCt++) {
			for (discardCt = 4; discardCt < 6; discardCt++) {
				for (handCt = 8; handCt < MAX_HAND+100; handCt++) {
					...
					// submit the game to checkUpdateCoins
