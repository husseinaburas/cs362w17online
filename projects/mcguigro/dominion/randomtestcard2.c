...
int main(int argc, char* argv[])
{
	...
	for (i = 0; i < 200; i++)
	{
		...
		int cardPos = insertCard(&newState, village);
		assert(cardPos != -1);
...
void printTestResult(struct gameState* newState, struct gameState* oldState, int testNum, int currentPlayer)
{
	...
	else if (oldState->deckCount[currentPlayer] > 0 && oldState->deckCount[currentPlayer] - newState->deckCount[currentPlayer] != 1)
		printf("TEST %i FAILED: DECK SIZE INCORRECT: %d, %d\n", testNum, oldState->deckCount[currentPlayer], newState->deckCount[currentPlayer]);
	else if (newState->numActions - oldState->numActions != 1)
		printf("TEST %i FAILED: WRONG NUMBER OF ACTIONS: %d, %d\n", testNum, oldState->numActions, newState->numActions);
	...
	else if (oldState->deckCount[currentPlayer] > 0 &&
		newState->hand[currentPlayer][newState->handCount[currentPlayer] - 1] != oldState->deck[currentPlayer][oldState->deckCount[currentPlayer] - 1])
			printf("TEST %i FAILED: GAINED WRONG CARD\n", testNum);
	else
		printf("TEST %i SUCCESSFUL\n", testNum);
