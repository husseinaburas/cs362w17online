...
int main(int argc, char *argv[]){
	...
	for(k = 0; k < N; k++){
		...
		if(TESTSTATEMENTS){
			printf("card played, check 3 cards added and 1 removed\n");
		...
		if(assertTrue(pretest.handCount[curPlayer] + 2, state.handCount[curPlayer], 0) == 0){
		...
		if(assertTrue(pretest.deckCount[curPlayer] -3, state.deckCount[curPlayer], 0) == 0){
...
////////////////////////////////////////////////////end test run
	...
	printf("Verify that hand count increased by 2 (3 added, 1 removed\n");
	printf("Passed: %d\nFailed: %d\n", rndmTests.handcountTest[0], rndmTests.handcountTest[1]);

	printf("Verify that deck count decreased by 3\n");
