...
int main(int argc, char *argv[]){
	...
	for(k = 0; k < N; k++){
		...
		if(assertTrue(state.playedCards[state.playedCardCount-1], adventurer, 0) == 0){
		...
		if(assertTrue(pretest.numActions, state.numActions, 0) == 0){
		...
		if(assertTrue(trCount1 + 2, trCount2, 0) == 0){
			rndmTests.treasureTest[0]++;
		}else{
			rndmTests.treasureTest[1]++;
		}
	}//end main for loop
...
////////////////////////////////////////////////////end test run
	...
	printf("Verify numActions not altered\n");
	...
	printf("Verify that two treasure were added to the hand or properly stopped\n");
	printf("Passed: %d\nFailed: %d\n", rndmTests.treasureTest[0], rndmTests.treasureTest[1]);
	
	//printf("Verify %s card added to the playedCard pile\n", testCard);
	//printf("Passed: %d\nFailed: %d\n", rndmTests.playedcardpileTest[0], rndmTests.playedcardpileTest[1]);
