...
int main(){
	...
	//add village to hand
	state.hand[curPlayer][state.handCount[curPlayer]] = village;
	...
	//confirm village in hand before attempting to play
	for(i=0; i<state.handCount[curPlayer]; i++){
		//cardNumtoName(state.hand[curPlayer][i], &curCard);
		//printf("Card %d= %s\n", i, curCard);
		if(state.hand[curPlayer][i] == village){
			cardCnt++;
			handpos = i;
		}
	}
	...
	results = cardEffect(village, choice1, choice2, choice3, &state, handpos, &bonus);
	...
	printf("card played, check 1 cards added and village removed\n");
	...
	assertTrue(pretest.handCount[curPlayer], state.handCount[curPlayer], 0);
	...
	printf("check playedCard includes village in last spot\n");
	...
	assertTrue(state.playedCards[state.playedCardCount-1], village, 0);
	...
	printf("check that numActions is increased by 2");
	printf("expected numActions: %d, numActions %d\n", pretest.numActions +2, state.numActions);
	assertTrue(pretest.numActions +2, state.numActions, 0);
