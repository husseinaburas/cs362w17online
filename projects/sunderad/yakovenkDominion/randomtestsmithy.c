...
int main(int argc, char const *argv[]){
	...
	int smithy_position;                 //Random spot in hand for smithy card 
	...
	for(i=0; i<TEST_LENGTH; i++){
		...
		smithy_position = ((int) floor(Random() * (G.handCount[G.whoseTurn] - 1)));
		G.hand[G.whoseTurn][smithy_position] = smithy;
		...
		playCard(smithy_position, 0, 0, 0, &G);
		...
		myAssert(msg1, i, (deck_pre - 3), deck_post);
		myAssert(msg2, i, (hand_pre + 2), hand_post);
