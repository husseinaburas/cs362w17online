...
int main() {
    ...
    int council[MAX_HAND];
    int silvers[MAX_HAND];
    int golds[MAX_HAND];
    for (i = 0; i < MAX_HAND; i++)
    {
	council[i] = council_room; 
        coppers[i] = copper;
        silvers[i] = silver;
        golds[i] = gold;
    ...
    for (p = 0; p < numPlayer; p++)
    {
        for (handCount = 1; handCount <= maxHandCount; handCount++)
        {
            for (bonus = 0; bonus <= maxBonus; bonus++)
            {
				...
				//my addition
				memcpy(G.hand[p],council,sizeof(int) * handCount); // set all the cards to gold
                updateCoins(p, &G, bonus);
#if (NOISY_TEST == 1)
                printf("G.coins = %d, expected = %d", G.coins,bonus);
#endif
                if(G.coins == bonus)
					printf("PASSED!\n");
				else 
