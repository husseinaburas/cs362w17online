...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case baron:
      return cardBaron(state, int choicel);
		
    case great_hall:
      return cardGreat_hall
    ...
    case cutpurse:
		return cardCutpurse(state, handPos);
...
int cardGreat_hall(struct gameState *state, int handPos){
      //+1 Card
      drawCard(currentPlayer, state);
...
int cardBaron(struct gameState *state, int choicel)(
