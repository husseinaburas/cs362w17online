...
int testCardSmithy (struct gameState *G, struct gameState *copyG, int player, int handPos) {
   ...
   assert_custom(G->hand[player][handPos] != smithy, "test smithy is removed from hand");
...
int main (int argc, char *argv[]) {
    ...
    for(i = 0; i < TEST_RUNS; i++)
    {
        ...
        // confirm smithy card is in hand
        assert_custom(G.hand[player][handPos] == smithy, "test smithy card is in hand");
        ...
        testCardSmithy(&G, &copyG, player, handPos);
        
        printf("---------------- End of Smithy Card Test Run# %i ----------------\n", i);
    }
