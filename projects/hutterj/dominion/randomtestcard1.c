...
Author: hutterj
Date: 2017.02.19
File: randontestcard1.c
Purpose: perform tests on semi-random game state for smithy card
...
Diff after played:
	...
	hand:
		doesn't have smithy from before
		has 3 new cards on top
	handCount = preState +2 (ie smithy is discarded, then get 3 cards)
	deckCount[] = -3 from before (need to account for empty deck shuffle)
	discard now smithy on top
	discardCount = preState +1
	playedCards[] top is smithy
...
int main(int argc, char *argv[]){
	...
	printf("Begin testing for smithy using seed %d\n", seed);
...
int oracle(struct gameState game){
	...
	if (preState.deckCount[thisPlayer] >= 3){
		asserttrue((game.deckCount[thisPlayer]+3 == preState.deckCount[thisPlayer]), "deckCount -3 from preState (not shuffled)");
	}
	else {
		...
		asserttrue((game.deckCount[thisPlayer]+3 == shuffledDeckCount), "deckCount -3 from preState (shuffled)");
	}
	...
	asserttrue((game.playedCards[game.playedCardCount-1] == CARD), "last played card is smithy");
	asserttrue((game.discard[thisPlayer][game.discardCount[thisPlayer]-1] == CARD), "last discarded card is smithy");
	
	// if the old top of deck wasn't smithy, smithy shouldn't be in same handpos as when it was played
	if (preState.deck[thisPlayer][preState.deckCount[thisPlayer]-1] != CARD){
		asserttrue((game.hand[thisPlayer][handPos] != CARD), "smithy's preState hand pos no longer has smithy");
	}
