...
void great_hallRandomTest(int *passed, int *failed){
    ...
    //cardEffect(great_hall, 0, 0, 0, &test, test.handCount[currentPlayer] - 1, 0);       //A5 - REFACTORED:  Teammate eisenbad created a separate method for great_hall outside of switch statement.  Need to use that function instead of passing great_hall into cardEffect()
    
    great_hall_refact(&test, test.handCount[currentPlayer] - 1);                         //A5 - REFACTORED:  New parameters are #1 - gameState struct, and #2 - handPos)
