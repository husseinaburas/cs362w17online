...
int main() {
	...
	{
		for (count = 1; count < MAX_DECK-490; count++)
		{
			for (handPos = 0; handPos < count; handPos++)
			{
				...
				if (state.hand[player][handPos] != 99)
					failFlag = 1;
				//player discarded village (handCount[player]--) and drew one new card (handCount[player]++) so hand count stays the same
#if (NOISY_TEST == 1)
				...
				if (state.deckCount[player] != count - 1)
					failFlag = 1;
				// player's number of actions should increase by one. the numActions decrement for playing village doesn't occur until control is returned to playCard()
				// which is not part of this test, so it is correct to only count the increase to actions here.
#if (HALTING_TEST == 1)
				assert(state.numActions == numActions + 2);
#endif
	...
	if (failFlag == 1)
		printf("cardtest4 (VILLAGE) - TESTS FAILED\n");
	else
		printf("cardtest4 (VILLAGE) - ALL TESTS PASSED\n");
