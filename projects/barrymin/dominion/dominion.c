  ...
  for (i = 0; i < state->handCount[player]; i++)
    {
      ...
      if (state->hand[player][i] == gardens) { score = score + state->deckCount[player] / 10 ); };
    }
  ...
  for (i = 0; i < state->discardCount[player]; i++)
    {
      ...
      if (state->discard[player][i] == gardens) { score = score + ( state->deckCount[player] / 10 ); };
    }
  ...
  for (i = 0; i < state->deckCount[player]; i++)
    {
      ...
      if (state->deck[player][i] == gardens) { score = score + ( state->deckCount[player] / 10 ); };
    }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus){
  ...
  switch( card )
    ...
    case baron:
      return playBaron(state,choice1);

    case great_hall:
      return playGreat_hall(state,handPos);

    case minion:
      return playMinion(state,handPos,choice1,choice2);
    ...
    case ambassador:
      return playAmbassador(state,handPos,choice1,choice2);
    ...
    case outpost:
      return playOutpost(state,handPos,choice1,choice2);
