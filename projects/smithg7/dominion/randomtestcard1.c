...
int main(int argc, char** argv) {
	...
	for(i = 0; i < NUMTESTS; i++)
	{
		...
		*************************************************************/
		...
		handleSmithy(thisPlayer, &testedState, 0);

		testSuccess[0] += assertTrue(testedState.whoseTurn, baseState.whoseTurn);
		testSuccess[1] += assertTrue(testedState.handCount[thisPlayer], baseState.handCount[thisPlayer] + 2);
		testSuccess[2] += assertTrue(testedState.deckCount[thisPlayer], baseState.deckCount[thisPlayer]);
		...
		testSuccess[7] += assertTrue(testedState.numActions, baseState.numActions);
