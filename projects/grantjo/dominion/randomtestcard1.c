...
** Description: Random test suite for the Village cardEffect in dominion.c
*******************************************************************************/
#include "dominion.h"
...
int refPlayVillage(struct gameState *state, struct villageDetails *detail, int handPos) {
...
// Description: Takes an unalterred game state and a state where Village
...
void performRandomTest(struct test *totalTest, int test, int rand_seed) {
  ...
  struct villageDetails villDetails;
  ...
  // calculate card pile sizes for player
  ...
  villDetails.currentPlayer = currentPlayer;
  testG.numPlayers = numPlayers;

  testG.handCount[currentPlayer] = handSize;
  // fill discard with random (not Village) cards and 1 village
  if (handSize > 0) {
  ...
  // fill deck with random (not Village) cards
  ...
  // fill discard with random (not Village) cards
  ...
  villDetails.tempHandCount = refG.handCount[currentPlayer];
  // Call Reference Logic on reference state
  refPlayVillage(&refG, &villDetails, handPos);
  ...
  // assert that reference Village logic is working correctly.
    ...
    asserttrue(refG.handCount[currentPlayer] == villDetails.tempHandCount, "Expected handCount to be the same after reference Village logic, check test");
    asserttrue(refG.playedCards[refG.playedCardCount-1] == cardToTest, "Village should be in played pile");
    ...
    asserttrue(seed == 0, "Village should not be in hand, check reference logic");
    asserttrue(refG.numActions == 3, "Num Actions should have increased by 2, check reference logic");
  ...
  printf("\tExpecting 1 card to have been added to the hand, got %d\n", testG.handCount[currentPlayer]-(villDetails.tempHandCount-1));
  error += asserttrue(testG.handCount[currentPlayer]-(villDetails.tempHandCount-1) == 1,
        "Should have drawn 1 card");

  if (!villDetails.shuffleOccurred) {
  ...
  error += asserttrue((testG.playedCards[testG.playedCardCount-1] == cardToTest) == 1,
          "Expected Village in played pile");
  ...
  error += asserttrue(seed == 0, "Village should not be in hand");
...
int main(int argc, char **argv) {
  ...
  if (total.testsFailed == 0)
      printf("ALL %d TESTS SUCCESSFUL, Village\n", total.testsPassed);
  else
      printf("%d TESTS FAILED, %d TESTS PASSED Village\n", total.testsFailed, total.testsPassed);
