...
int main(int argc, char** argv)
{
  ...
  int i = 0,
      ...
      tempDeck[MAX_DECK],       // store deck to check that Village cards come from there
      tempDeckCount,
      tempHandCount;

  // Village reference logic helper variables
  int currentPlayer = 1,	// track current player
      z = 0,				     // track size of temphand
	    handPos = 0;			// position of Village card
...
//******************************* TEST 1 - Default Deck (Just Initialized) Village on player 2 *********************************
  printf("Test 1 Village, Player 2 default deck, 10 cards in deck, 0 in discard, and only Village in hand\n");

  // Add Village to player 2's hand
  ...
  // Store altPlayers hand to check after call to Village
  ...
  //call cardEffect for Village on player
  success = cardEffect(village, -1, -1, -1, &testG, handPos, &bonus);

  //******************************* APPLY Village LOGIC TO REFERENCE STATE *********************************
  // Here we implement the logic of Village
  ...
  //******************************* END Village LOGIC TO REFERENCE  *********************************

  // assert that reference Village logic is working correctly.
  // Does not count towards error_count for dominion.c code
  asserttrue(G.handCount[currentPlayer] == 1, "Expected handCount to be 1 after reference Village logic, check test");
  asserttrue(G.playedCards[G.playedCardCount-1] == village, "Village should be in played pile");
  // make sure Village is not in hand
  z = 0;
  for (i = 0; i < G.handCount[currentPlayer]; i++ ) {
	  if (G.hand[currentPlayer][i] == village)
		  z++;
  }
  asserttrue(z == 0, "Village should not be in hand, check reference logic");
  asserttrue(G.numActions == 3, "Num Actions should have increased by 2, check reference logic");
  // ********** End checking reference Village logic

  //*********************** LOGIC TO CHECK Village doesnt effect other players deck or other gamestate vars **************/
  printf("\tExpecting Village does not alter other players deck or irrelevent game state variables\n");
  ...
  for (i = 0; i < altHandCount; i++)
	  if (altHand[i] == testG.hand[alternatePlayer][i])
		  z++;
  error_count += asserttrue(z == altHandCount, "Village changed another players hand");
  ...
  for (i = 0; i < altDeckCount; i++)
	  if (altDeck[i] == testG.deck[alternatePlayer][i])
		  z++;
  error_count += asserttrue(z == altDeckCount, "Village changed another players deck");
  ...
  error_count += asserttrue(z == altDiscardCount, "Village changed another players discard");
  ...
  error_count += asserttrue(z == treasure_map+1, "Village changed supplyCount");
  ...
  error_count += asserttrue(z == treasure_map+1, "Village changed embargoTokens");

  // Assert altHandCount, altDeckCount, and altDiscardCount match current alternate players counts
  error_count += asserttrue(testG.handCount[alternatePlayer] == altHandCount, "Village changed another players handCount");
  error_count += asserttrue(testG.deckCount[alternatePlayer] == altDeckCount, "Village changed another players deckCount");
  error_count += asserttrue(testG.discardCount[alternatePlayer] == altDiscardCount, "Village changed another players handCount");
  // assert that other gameState variables have not changed
  error_count += asserttrue(numPlayers == testG.numPlayers, "Village change numPlayers");
  error_count += asserttrue(outpostPlayed == testG.outpostPlayed, "Village change outpostPlayed");
  error_count += asserttrue(outpostTurn == testG.outpostTurn, "Village change outpostTurn");
  error_count += asserttrue(whoseTurn == testG.whoseTurn, "Village change whoseTurn");
  error_count += asserttrue(phase == testG.phase, "Village change phase");
  error_count += asserttrue(coins == testG.coins, "Village change coins");
  error_count += asserttrue(numBuys == testG.numBuys, "Village change numBuys");

  if (error_count == 0)
	  printf("\t\tSuccess: Village does not change other players hand or irrelevant game state variables\n");
  //*********************** END OF LOGIC TO CHECK Village doesnt effect other players deck **************/

  // assert cardEffect returns 0
  error_count += asserttrue(success == 0, "Call to cardEffect for Village returned status failed");
  ...
  // keep track of whether returned hand has Village
  seed = 0;
  // Check returned hand doesnt have Village
  for (i = 0; i < testG.handCount[currentPlayer]; i++) {
    if (testG.hand[currentPlayer][i] == village)
      seed++;
  }
  // print expected value of Village card in hand
  printf("\tExpects 0 if returned hand doesnt have Village card and got %d\n", seed);
  error_count += asserttrue(seed == 0, "Village should not be in hand");
  
  printf("\tExpects 1 if village is in played pile and got %d\n", testG.playedCards[G.playedCardCount-1] == village);
  error_count += asserttrue(testG.playedCards[G.playedCardCount-1] == village, "Village should be in played pile");
  ...
  //******************************* TEST 2 - Player1 deck empty, discard has 12 cards with, 3 copper in hand with Village in pos 1 ***********************
  printf("Test 2 Village, Player1 discard empty, deck has 12 cards with, 3 copper in hand with Village in pos 1\n");

  // clear Village variables and make currentPlayer player 1
  currentPlayer = 0;
  z = 0;
  ...
  G.hand[currentPlayer][G.handCount[currentPlayer]++] = village;
  G.hand[currentPlayer][G.handCount[currentPlayer]++] = copper;
  G.hand[currentPlayer][G.handCount[currentPlayer]++] = copper;
  handPos = 1;
  tempHandCount = G.handCount[currentPlayer];
  ...
  // assert that reference Village logic is working correctly.
  // Does not count towards error_count for dominion.c code
  asserttrue(G.handCount[currentPlayer] == tempHandCount, "Expected handCount to be the same after reference Village logic, check test");
  asserttrue(G.playedCards[G.playedCardCount-1] == village, "Village should be in played pile");
  // make sure Village is not in hand
  z = 0;
  for (i = 0; i < G.handCount[currentPlayer]; i++ ) {
  ...
  if (error_count == 0)
	  printf("\t\tSuccess: Village does not change irrelevant game state variables\n");
  //*********************** END OF LOGIC TO CHECK Village doesnt effect other players deck **************/
  ...
  if (error_count == 0)
    printf("\n\tAll Tests Successful, Test 2 Village, Player1 discard empty, deck has 12 cards with, 3 copper in hand with Village in pos 1\n\n");
  else
    printf("\n\t%d Tests Failed, Test 2 Village, Player1 discard empty, deck has 12 cards with, 3 copper in hand with Village in pos 1\n\n", error_count);

  error_total += error_count;
  error_count = 0;

  //******************************* TEST 3 - Village, Player 2 empty deck, 12 cards in discard, hand has 2 silver with Village on top ***********************
  printf("Test 3 Village, Player 2 empty deck, 12 cards in discard, hand has 2 silver with Village on top\n");
  ...
  G.hand[currentPlayer][G.handCount[currentPlayer]++] = 5;
  G.hand[currentPlayer][G.handCount[currentPlayer]++] = 14;
  handPos = 2;
  tempHandCount = G.handCount[currentPlayer];
  G.numActions = 1;
  ...
  // assert that reference Village logic is working correctly.
  // Does not count towards error_count for dominion.c code
  asserttrue(G.handCount[currentPlayer] == tempHandCount , "Expected handCount be the same after reference Village logic, check test");
  asserttrue(G.playedCards[G.playedCardCount-1] == village, "Village should be in played pile");
  // make sure Village is not in hand
  z = 0;
  for (i = 0; i < G.handCount[currentPlayer]; i++ ) {
  ...
  if (error_count == 0)
    printf("\n\tAll Tests Successful, Test 3 Village, Player 2 empty deck, 12 cards in discard, hand has 2 silver with Village on top\n\n");
  else
    printf("\n\t%d Tests Failed, Test 3 Village, Player 2 empty deck, 12 cards in discard, hand has 2 silver with Village on top\n\n", error_count);

  error_total += error_count;

  if (error_total == 0)
    printf("ALL TESTS SUCCESSFUL, Village\n");
  else
    printf("%d TESTS FAILED, Village\n", error_total);

  printf("------------------------ END Test Village Card ------------------------------\n");
