...
if(coins == 0)
if((current->handCount[0]) != priorHandCount-1)
...
if(coins == 1)
if(((current->handCount[0]) != (priorHandCount)) || (current->hand[0][current->handCount[0]] != flags[6]))
	abortDump(current, 1, flags);
//this one does the same as above, but with 2 coins. 
if(coins == 2)
if(((current->handCount[0]) != (priorHandCount+1)) || (current->hand[0][current->handCount[0]] != flags[7]) || (current->hand[0][current->handCount[0]-1] != flags[6]))  
	abortDump(current, 2, flags); //if the aibove fails something bad happened to adventurer, dump the gameState
}
//doesn't work, adventurer doesnt discard itself
//we made it
