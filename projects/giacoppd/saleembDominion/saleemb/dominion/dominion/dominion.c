...
void adventurerfunc(int currentPlayer, struct gameState *state, int handPos, int drawntreasure, int cardDrawn, int z, int * temphand)
{
...
void baronfunc(int currentPlayer, struct gameState *state, int handPos, int choice1){
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
     	adventurerfunc(currentPlayer, state, handPos, drawntreasure, cardDrawn, z, temphand);
	return 0;	
    case council_room:
      //+4 Cards
      for (i = 0; i < 4; i++)
    ...
    case feast:
      feastfunc(currentPlayer, state, handPos, temphand, choice1);
      return 0;
	...
	return 0;
    case baron:
	baronfunc(currentPlayer, state, handPos, choice1);
	return 0;
    case great_hall:
      //+1 Card
      drawCard(currentPlayer, state);
...
int discardCard(int handPos, int currentPlayer, struct gameState *state, int trashFlag)
{
  ...
  //remove card from player's hand (THIS BIT IS FOR KEEPPING ARRAYS TIDY)
